...
use std::time::Duration;fn main() {
    loop {
        ...
        let mut attempts = 0;
        let max_attempts = 2;        while attempts < max_attempts {
            println!("There are 5 wires in front of you recruit. Which wire do you want to cut? (Hint: Enter 1-5):");            // Read user input
            let mut guess = String::new();
            io::stdin()
                .read_line(&mut guess)
                .expect("Failed to read line");            // Parse input to a number
